...
export const getUserById = async (userId) => {
  try {
    ...
    if (error) {
      ...
      return { success: false, error: error, isNotFoundError: error.code === 'PGRST116' };
    }
    ...
    return { success: false, error: error, isNotFoundError: false };
  }
};
...
export const createSubmission = async (teamMemberName, submissionLink, problemDescription, hoursSpent, projectName, servicesUsed, gitRepoUrl, category) => {
  try {
    const { data, error } = await supabase
      .from('submissions')
      .insert([
        {
          ...
          category: category || 'Via Cursor Project'
        }
      ])
      .select('*')
      .single();
